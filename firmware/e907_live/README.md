...
接舵机观察转动请使用新增的 `--bench-servo 1|2|3|4|all`，完整接线与命令见 [独立台架测试](BENCH.md)。已完成单舵机小幅/增幅以及四个已接舵机依次测试；日志指令和寄存器检查通过。四路同时模式已上传，但尚无实机运行记录。详见 [当日总结及图片](../../reports/servo_test_summary_2026-09-18.md)。
...
后续已接舵机：单路小幅录像可见机械响应，单路增幅与四路依次测试有通过日志；不能据此认定角度精度或负载能力达标。四路同时运行、长期保持，以及四个 H2 引脚的电平、真实周期、脉宽和更新毛刺仍未验证。各次证据见 [完整总结](../../reports/servo_test_summary_2026-09-18.md)。

当前日志采集支持新旧格式，时间含义和 CSV 使用见 [TELEMETRY.md](TELEMETRY.md)。
